...
export default function SessionsPage() {
  ...
  const convertAudioToWav = useCallback(async (audioBlob: Blob): Promise<Blob> => {
    console.log("🔄 FRONTEND: Converting audio to WAV format...")
    ...
    console.log("✅ FRONTEND: Audio converted to 16kHz mono WAV")
    return audioBufferToWav(renderedBuffer)
  }, [])

  const startRecording = useCallback(async () => {
    try {
      console.log("🎤 FRONTEND: Starting real-time recording...")
      ...
      mediaRecorderRef.current.onstop = async () => {
        console.log("⏹️ FRONTEND: Recording stopped, processing audio...")
        ...
        try {
          ...
          console.log("✅ FRONTEND: Recording ready for processing")
        } catch (error) {
          toast.error("Failed to convert audio.")
          console.error("❌ FRONTEND: Audio conversion error:", error)
        }
      ...
      console.error("❌ FRONTEND: Error accessing microphone:", err)
    }
  }, [convertAudioToWav])

  const stopRecording = useCallback(() => {
    if (mediaRecorderRef.current && mediaRecorderRef.current.state === "recording") {
      console.log("⏹️ FRONTEND: Stopping recording...")
  ...
  const handleFileUpload = useCallback(
    async (file: File) => {
      console.log("📁 FRONTEND: File uploaded:", file.name, file.size, "bytes")
      ...
      try {
        ...
        console.log("✅ FRONTEND: File ready for processing")
      } catch (error) {
        toast.error("Failed to convert uploaded file.")
        console.error("❌ FRONTEND: File conversion error:", error)
        setUploadedFile(null)
      }
    },
    [convertAudioToWav],
  ...
  const handleProcessing = async () => {
    ...
    console.log("🚀 FRONTEND: Starting AI processing pipeline...")
    setStage("processing")
    setProcessingProgress(0) // Reset progress

    const progressSteps = [
      { progress: 10, message: "📤 Uploading audio to backend..." },
      { progress: 25, message: "🔧 Configuring AI pipeline..." },
      { progress: 45, message: "🎵 Converting speech to text..." },
      { progress: 65, message: "🌐 Translating to target language..." },
      { progress: 85, message: "🧠 Generating AI summary..." },
      { progress: 95, message: "📋 Extracting action items..." },
    ...
    const progressInterval = setInterval(() => {
      if (currentProgressIndex < progressSteps.length - 1) {
        ...
        console.log("📊 FRONTEND:", progressSteps[currentProgressIndex].message)
    ...
    try {
      ...
      formData.append("primaryLanguage", selectedPrimaryLanguage.split("-")[0]) // Convert hi-IN to hi
      formData.append("targetLanguage", selectedTargetLanguage.split("-")[0]) // Convert en-US to en
      formData.append("preMeetingNotes", preMeetingNotes)

      console.log(`🌐 FRONTEND: Sending request to backend: /api/process-audio/`)
      console.log("📋 FRONTEND: Request details:")
      console.log("   - Audio file:", audioSource.name || "recorded-audio")
      console.log("   - Primary language:", selectedPrimaryLanguage)
      console.log("   - Target language:", selectedTargetLanguage)
      console.log("   - Pre-meeting notes:", preMeetingNotes ? "Yes" : "No")
      ...
      console.log("📡 FRONTEND: Response received from backend:", response.status)
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error("❌ FRONTEND: Non-JSON error response")
        }
      ...
      console.log("📊 FRONTEND: Results received:")
      console.log("   - Transcript length:", data.transcript?.length || 0, "characters")
      console.log("   - Translation length:", data.translatedText?.length || 0, "characters")
      console.log("   - Summary length:", data.summary?.length || 0, "characters")
      console.log("   - Action items:", data.actionItems?.length || 0, "items")
      ...
      if (error.name === "TypeError" && error.message.includes("fetch")) {
        ...
        console.error("❌ FRONTEND: Backend connection failed")
      } else if (error.message) {
        errorMessage = error.message
        console.error("❌ FRONTEND: Processing error:", error.message)
      }

      toast.error(errorMessage)
      console.error("❌ FRONTEND: Error during processing:", error)
    }
  }

  const resetSession = () => {
    console.log("🔄 FRONTEND: Resetting session...")
    setStage("upload")
    setRecordedAudioBlob(null)
    setUploadedFile(null)
    setProcessingProgress(0)
